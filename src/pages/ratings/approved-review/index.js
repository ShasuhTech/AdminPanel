...
import AprovalModal from "./ApproveModal";
...
const ApprovedReview = () => {
  ...
  const getApprovedRating = async (page) => {
    ...
    try {
      ...
      const res = await axiosInstance.get(
        `admin/ratings/list?status=2&q=${searchText || ""}&page=${page || ""}`
      );
  ...
  console.log(selectedData, "selectedDataparent");
...
const Row = (props) => {
  ...
  const renderStars = (rating) => {
    ...
    for (let i = 1; i <= 5; i++) {
      if (i <= rating) {
        stars.push(<StarIcon key={i} sx={{ color: "black" }} />);
      }
    }
    return stars;
  };
