const ClassList = [
  {
    value: "Play School",
    label: "Play School",
  },
  {
    value: "Nursery",
    label: "Nursery",
  },
  {
    value: "LKG",
    label: "LKG",
  },
  {
    value: "UKG",
    label: "UKG",
  },
  {
    value: "1",
    label: "1",
  },
  {
    value: "2",
    label: "2",
  },
  {
    value: "3",
    label: "3",
  },
  {
    value: "4",
    label: "4",
  },
  {
    value: "5",
    label: "5",
  },
  {
    value: "6",
    label: "6",
  },
  {
    value: "7",
    label: "7",
  },
  {
    value: "8",
    label: "8",
  },
  {
    value: "9",
    label: "9",
  },
  {
    value: "10",
    label: "10",
  },
  {
    value: "11",
    label: "11",
  },
  {
    value: "12",
    label: "12",
  },
];

const SectionList = [
  {
    value: "A",
    label: "A",
  },
  {
    value: "B",
    label: "B",
  },
  {
    value: "C",
    label: "C",
  },
  {
    value: "D",
    label: "D",
  },
  {
    value: "E",
    label: "E",
  },
  {
    value: "F",
    label: "F",
  },
];

const StreamList = [
  {
    value: "Science",
    label: "Science",
  },
  {
    value: "Arts",
    label: "Arts",
  },
  {
    value: "Maths",
    label: "Maths",
  },
];
const ModeOfFollowUp = [
  {
    value: "Telephone",
    label: "Telephone",
  },
  {
    value: "WhatsApp",
    label: "WhatsApp",
  },
  {
    value: "Offline",
    label: "Offline",
  },
];
const FeeGroup = [
  {
    value: "1",
    label: "I-V",
  },
  {
    value: "2",
    label: "VI-VIII",
  },
  {
    value: "3",
    label: "IX-X",
  },
];
const FeeApplyFrom = [
  {
    value: "1",
    label: "QTR 1",
  },
  {
    value: "2",
    label: "QTR 2",
  },
  {
    value: "3",
    label: "QTR 3",
  },
];

const AdmissionType = [
  {
    value: "1",
    label: "New",
  },
  {
    value: "2",
    label: "Old",
  },
];
const IndianReligions = [
  {
    value: "1",
    label: "Hinduism",
  },
  {
    value: "2",
    label: "Islam",
  },
  {
    value: "3",
    label: "Christianity",
  },
  {
    value: "4",
    label: "Sikhism",
  },
  {
    value: "5",
    label: "Jainism",
  },
];
const SocialCategories = [
  {
    value: "1",
    label: "General",
  },
  {
    value: "2",
    label: "OBC",
  },
  {
    value: "3",
    label: "SC",
  },
  {
    value: "4",
    label: "ST",
  },
  {
    value: "5",
    label: "EWS",
  },
  // Add more social categories as needed
];
const AllCastes = [
  {
    value: "1",
    label: "Brahmin",
    category: "Hindu",
  },
  {
    value: "2",
    label: "Kshatriya",
    category: "Hindu",
  },
  {
    value: "3",
    label: "Vaishya",
    category: "Hindu",
  },
  {
    value: "4",
    label: "Shudra",
    category: "Hindu",
  },
  {
    value: "5",
    label: "Jat",
    category: "Hindu",
  },
  {
    value: "1",
    label: "Sayyid",
    category: "Muslim",
  },
  {
    value: "2",
    label: "Sheikh",
    category: "Muslim",
  },
  {
    value: "3",
    label: "Mughal",
    category: "Muslim",
  },
  {
    value: "4",
    label: "Pathan",
    category: "Muslim",
  },
  {
    value: "5",
    label: "Ansari",
    category: "Muslim",
  },
  // Add more caste objects as needed
];
const MaritalStatus = [
  {
    value: "1",
    label: "Single",
  },
  {
    value: "2",
    label: "Married",
  },
  {
    value: "3",
    label: "Divorced",
  },
  {
    value: "4",
    label: "Widowed",
  },
  {
    value: "5",
    label: "Separated",
  },
  // Add more marital statuses as needed
];
const MotherTongues = [
  {
    value: "1",
    label: "Hindi",
  },
  {
    value: "2",
    label: "English",
  },
  {
    value: "3",
    label: "Bengali",
  },
  {
    value: "4",
    label: "Telugu",
  },
  {
    value: "5",
    label: "Marathi",
  },
  // Add more mother tongues as needed
];
const CountryDta = [
  {
    value: "1",
    label: "India",
  },
  {
    value: "2",
    label: "Pakistan",
  },
  {
    value: "3",
    label: "China",
  },
  {
    value: "4",
    label: "Dubai",
  },
  {
    value: "5",
    label: "UAE",
  },
  // Add more mother tongues as needed
];
const SchoolConcessionTypes = [
  {
    value: "1",
    label: "Scholarship",
  },
  {
    value: "2",
    label: "Financial Aid",
  },
  {
    value: "3",
    label: "Free Lunch Program",
  },
  {
    value: "4",
    label: "Transportation Subsidy",
  },
  {
    value: "5",
    label: "Book Allowance",
  },
  // Add more concession types for school students as needed
];
const Nationalities = [
  {
    value: "1",
    label: "Indian",
  },
  {
    value: "2",
    label: "American",
  },
  {
    value: "3",
    label: "British",
  },
  {
    value: "4",
    label: "Chinese",
  },
  {
    value: "5",
    label: "French",
  },
  // Add more nationalities as needed
];
const BloodGroups = [
  {
    value: "1",
    label: "A+",
  },
  {
    value: "2",
    label: "A-",
  },
  {
    value: "3",
    label: "B+",
  },
  {
    value: "4",
    label: "B-",
  },
  {
    value: "5",
    label: "AB+",
  },
  {
    value: "6",
    label: "AB-",
  },
  {
    value: "7",
    label: "O+",
  },
  {
    value: "8",
    label: "O-",
  },
  // Add more blood groups as needed
];
const BoardingCategories = [
  {
    value: "1",
    label: "Day Scholar",
  },
  {
    value: "2",
    label: "Day Boarder",
  },
  {
    value: "3",
    label: "Hosteler",
  },
  {
    value: "4",
    label: "Weekly Boarder",
  },
  {
    value: "5",
    label: "Monthly Boarder",
  },
  // Add more boarding categories as needed
];
const Boards = [
  {
    value: "1",
    label: "CBSE (Central Board of Secondary Education)",
  },
  {
    value: "2",
    label: "ICSE (Indian Certificate of Secondary Education)",
  },
  {
    value: "3",
    label: "State Board",
  },
  {
    value: "4",
    label: "IB (International Baccalaureate)",
  },
  {
    value: "5",
    label: "IGCSE (International General Certificate of Secondary Education)",
  },
  // Add more boards as needed
];
const PaymentTypes = [
  {
    value: "1",
    label: "Credit Card",
  },
  {
    value: "2",
    label: "Debit Card",
  },
  {
    value: "3",
    label: "Net Banking",
  },
  {
    value: "4",
    label: "UPI (Unified Payments Interface)",
  },
  {
    value: "5",
    label: "Wallet",
  },
  {
    value: "6",
    label: "Cash",
  },
  {
    value: "7",
    label: "Cheque",
  },
  {
    value: "8",
    label: "Demand Draft (DD)",
  },
  // Add more payment types as needed
];

const HouseTypes = [
  {
    value: "1",
    label: "Detached House",
  },
  {
    value: "2",
    label: "Semi-detached House",
  },
  {
    value: "3",
    label: "Terraced House",
  },
  {
    value: "4",
    label: "Apartment/Flat",
  },
  {
    value: "5",
    label: "Bungalow",
  },
  {
    value: "6",
    label: "Townhouse",
  },
  // Add more house types as needed
];
const SchoolBusOptions = [
  {
    value: "1",
    label: "Available",
  },
  {
    value: "2",
    label: "Not Available",
  },
  // Add more options as needed
];
const qualifications = [
  {
    value: "1",
    label: "Graduation",
  },
  {
    value: "2",
    label: "Post Graduation",
  },
  {
    value: "3",
    label: "Doctorate",
  },
  {
    value: "4",
    label: "Diploma",
  },
  {
    value: "5",
    label: "Certificate",
  },
  {
    value: "6",
    label: "High School",
  },
  {
    value: "7",
    label: "Middle School",
  },
  {
    value: "8",
    label: "Primary School",
  },
];
const occupations = [
  {
    value: "1",
    label: "Engineer",
  },
  {
    value: "2",
    label: "Doctor",
  },
  {
    value: "3",
    label: "Teacher",
  },
  {
    value: "4",
    label: "Artist",
  },
  {
    value: "5",
    label: "Scientist",
  },
  {
    value: "6",
    label: "Lawyer",
  },
  {
    value: "7",
    label: "Accountant",
  },
  {
    value: "8",
    label: "Nurse",
  },
  {
    value: "9",
    label: "Pharmacist",
  },
  {
    value: "10",
    label: "Architect",
  },
  {
    value: "11",
    label: "Software Developer",
  },
  {
    value: "12",
    label: "Business Analyst",
  },
  {
    value: "13",
    label: "Journalist",
  },
  {
    value: "14",
    label: "Pilot",
  },
  {
    value: "15",
    label: "Chef",
  },
  {
    value: "16",
    label: "Electrician",
  },
  {
    value: "17",
    label: "Plumber",
  },
  {
    value: "18",
    label: "Mechanic",
  },
  {
    value: "19",
    label: "Manager",
  },
  {
    value: "20",
    label: "Entrepreneur",
  },
];

const joiningYear = [
  {
    value: "1",
    label: "2024",
  },
  {
    value: "2",
    label: "2023",
  },
  {
    value: "3",
    label: "2022",
  },
  {
    value: "4",
    label: "2021",
  },
];

const followupStatus = [
  {
    value: "3",
    label: "All",
  },
  {
    value: "1",
    label: "Follow Up",
  },
  {
    value: "2",
    label: "Registration",
  },
];

const genders = [
  {
    value: "1",
    label: "Male",
  },
  {
    value: "2",
    label: "Female",
  },
  {
    value: "3",
    label: "Other",
  },
];

export default {
  ClassList,
  SectionList,
  StreamList,
  FeeGroup,
  FeeApplyFrom,
  AdmissionType,
  IndianReligions,
  SocialCategories,
  AllCastes,
  MaritalStatus,
  MotherTongues,
  SchoolConcessionTypes,
  Nationalities,
  BloodGroups,
  BoardingCategories,
  Boards,
  PaymentTypes,
  HouseTypes,
  SchoolBusOptions,
  qualifications,
  joiningYear,
  followupStatus,
  occupations,
  genders,
  CountryDta,
  ModeOfFollowUp
};
